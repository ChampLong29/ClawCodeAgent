...
训练控制台用于浏览 rollout、查看消息时间线、触发 mock/真实模型 rollout，并导出 SFT/RL 数据集。演示时建议在项目根目录直接使用`.venv/bin`入口：
...
不建议把`uv run claw train-web ...`作为现场演示命令。`uv run`会先同步当前项目包，因此可能输出`Uninstalled 1 package`和`Installed 1 package`；这不是训练 Web 的业务错误，但会增加启动不确定性。
...
from claw.training.slime_adapter import SlimeDataAdapter
import json
with open('sft_cold_start.jsonl') as f: results = [json.loads(line) for line in f]
...
from claw.training.slime_adapter import SlimeDataAdapter
import json
with open('flywheel_round1.jsonl') as f: results = [json.loads(line) for line in f]
# 质量筛选
...
```text
Work Agent 产出: - write_file 记录 {路径: 内容}
  - test_commands 执行结果
  - 架构文档

  ↓

Reviewer Agent（独立 Session, 独立 System Prompt）: - "你是一个严格的代码审查专家"
  - 逐维度打分（0.0-1.0）
  - 逐问题列出（severity + file + description + suggestion）

  ↓

Combined Reward: reward = test_pass_rate × 0.4
  + diff_accuracy × 0.3
  + review_score × 0.3
...
for issue in report.issues: print(f"  [{issue.severity}] {issue.dimension}: {issue.description}")

```

### Reviewer 在 SLIME 中的角色

在 SLIME RL 训练中，Reviewer 的评分**不直接参与** on-policy reward 计算（因为 SLIME rollout 时 Reviewer 不能在线调用 API）。替代方案： 1. **Offline Review**（推荐）：Rollout 完成后，用 Reviewer 对所有 trajectory 打分，review_score 合并进 JSONL 的`reward` 字段
...
with open('flywheel_r1.jsonl') as f: results = [json.loads(line) for line in f]
