...
- **耦合度 (Coupling)**：从高到低
  ...
  - Common（共享全局变量）
  - Control（A 控制 B 的执行流）注意
  - Stamp（共享数据结构）注意
  - Data（仅传参数）
  - Message（消息传递）
- **内聚度 (Cohesion)**：从低到高
  - Coincidental（偶然放一起）
  - Logical（按类型分组，如所有 IO）注意
  - Temporal（同一时间执行）注意
...
- **POSIX 风格**：短选项 `-x`，长选项 `--xxx`- **GNU 风格**：长选项可以`--xxx=value`或`--xxx value`- **避免位置参数过多**：超过 2 个就考虑用 flag
...
- 用 `subprocess.run([cmd, arg1, arg2])`而非`subprocess.run(f"cmd {arg}", shell=True)`- 必须 shell=True 时用`shlex.quote()` 转义
...
- **Docker 最佳实践**： - 多阶段构建（builder + runtime 分离，减小镜像）
  - 层缓存优化（变化频繁的放后面）
  - 非 root 用户运行
