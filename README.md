...
- **任意阶段回滚**：基于快照的状态恢复，回滚日志可审计
- **阶段级上下文管理**：自动压缩历史阶段为结构化摘要
- **双 API 支持**：Anthropic 原生 Messages API + OpenAI 兼容 `/chat/completions`
- **交互式 REPL**：彩色终端 UI，实时流式输出，权限审批卡片
- **Agent 训练子系统**：Gym 风格环境、沙箱隔离、多进程批量 Rollout、SLIME 数据飞轮集成
...
# 3. 启动交互式 REPL（在目标项目目录下）
cd /path/to/your/project
claw agent-chat --cwd .
```

安装后 `claw` 命令在 venv 中全局可用，可在任意目录执行。
